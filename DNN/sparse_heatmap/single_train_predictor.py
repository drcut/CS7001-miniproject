...
def train(args, model, train_loader, loss_func, optimizer, epoch):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        data = data.view(-1, height, width, 1)
        output = model(data)
            ...
            if batch_idx % args.log_interval == 0:
                print(
                    "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} Error rate: {:.6f}".format(
                        ...
                        loss.item(),
                        torch.mean(torch.abs((output-target_reuse_dis)/(target_reuse_dis+1))),
                    )
                )
                if args.dry_run:
                    break
...
def test(model, rank, test_loader):
    ...
    with torch.no_grad():
        for batch_idx, sample_batched in enumerate(test_loader):
            ...
            import pdb
            pdb.set_trace()
            error_rate += torch.mean(torch.abs((output-target_reuse_dis)/target_reuse_dis+0.0001)) 
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=32, metavar="N", help="input batch size for training (default: 64)"
    )
    parser.add_argument(
