...
OS *os_new(uns num_pages, uns num_threads) {
  ...
  init_hash_table(os->pt->entries, (char *)"PageTableEntries", 4315027,
                  sizeof(PageTableEntry));
  os->pt->max_entries = os->num_pages;

  os->ipt = (InvPageTable *)calloc(1, sizeof(InvPageTable));
  os->ipt->entries =
      (InvPageTableEntry *)calloc(os->num_pages, sizeof(InvPageTableEntry));
...
uns os_vpn_to_pfn(OS *os, uns vpn, uns tid, Flag *hit) {
  ...
  *hit = TRUE;
  ...
  pte = (PageTableEntry *)hash_table_access_create(pt->entries, vpn,
                                                   &first_access);
