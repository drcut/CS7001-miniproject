...
DRAM *dram_new(uns64 memsize, uns64 num_channels, uns64 num_banks,
               uns64 rowbuf_size, uns64 t_ACT, uns64 t_CAS, uns64 t_RAS,
               uns64 t_RP, uns64 t_BURST) {
  uns ii;
  ...
  d->lines_in_rowbuf =
      rowbuf_size / LINESIZE; // DRAM access granularity is linesize
...
uns64 dram_service(DRAM *d, Addr lineaddr, DRAM_ReqType type,
                   double num_lineburst, uns64 in_cycle, ACTinfo *act_info) {
  ...
  // printf("line addr: %d    base: %d\n", lineaddr, d->lines_in_mem);
  ...
  if (mybank->valid && (mybank->rowbufid != myrowbufid)) {
    ...
    delay_rc =
        scoreboard_service(mybank->sb, in_cycle + delay + ras_wait, d->t_RP);
    delay += delay_rc;
    mybank->valid = FALSE;
  ...
  double dbus_time =
      d->t_BURST * num_lineburst; // assume constant latency BUS for now
  delay += dbus_time;
...
void dram_print_stats(DRAM *d) {
  ...
  uns64 rd_rowbuf_conflict =
      d->s_rowbuf_outcome[DRAM_REQ_RD][DRAM_ROWBUF_CONFLICT];
  ...
  if (d->s_access_type[DRAM_REQ_RD]) {
    rd_rbuf_hitperc = 100.0 * (double)(rd_rowbuf_hits) /
                      (double)(d->s_access_type[DRAM_REQ_RD]);
    rd_rbuf_emptyperc = 100.0 * (double)(rd_rowbuf_empty) /
                        (double)(d->s_access_type[DRAM_REQ_RD]);
    rd_rbuf_conflictperc = 100.0 * (double)(rd_rowbuf_conflict) /
                           (double)(d->s_access_type[DRAM_REQ_RD]);
    avg_rd_delay = (double)(d->s_delaysum_type[DRAM_REQ_RD]) /
                   (double)(d->s_access_type[DRAM_REQ_RD]);
  ...
  printf("\n%s_RDACCESS         \t : %llu", header,
         d->s_access_type[DRAM_REQ_RD]);
...
double dram_calc_avgwait(DRAM *d, DRAM_ReqType type) {
  ...
  sum_banktimes +=
      (d->s_rowbuf_outcome[type][DRAM_ROWBUF_EMPTY] * bank_empty_time);

  // sum of bank conflict
  uns64 bank_conf_time = d->t_ACT + d->t_CAS; // not including d->t_RP;
  sum_banktimes +=
      (d->s_rowbuf_outcome[type][DRAM_ROWBUF_CONFLICT] * bank_conf_time);
  ...
  double avgwait =
      (d->s_delaysum_type[type] - sum_bank_bus) / d->s_access_type[type];
...
void dram_parseaddr(DRAM *d, Addr lineaddr, uns64 *myrowbufid, uns64 *mybankid,
                    uns64 *mychannelid) {
  *myrowbufid = lineaddr / d->lines_in_rowbuf;

  *mybankid =
      *myrowbufid / d->rowbufs_in_bank; // consecutive rowbufs are in same bank;

  if (DRAM_STRIPE_ROWBUFS_TO_BANKS) {
    *mybankid =
        *myrowbufid % d->num_banks; // consecutive rowbufs go to diff bank
  }
  ...
  *mychannelid = *mybankid / d->banks_in_channel;
  ...
  if (DRAM_STRIPE_BANKS_TO_CHANNELS) {
    *mychannelid =
        *mybankid % d->num_channels; // consecutive banks go to diff channel
  }
}
...
void dram_get_neighbor_lineaddr(DRAM *d, uns64 rowbufid,
                                uns64 *row_prev_lineaddr,
                                uns64 *row_next_lineaddr) {
...
Addr dram_get_bankid(DRAM *d, uns64 rowbufid) {
  Addr bankid =
      rowbufid / d->rowbufs_in_bank; // consecutive rowbufs are in same bank;
  if (DRAM_STRIPE_ROWBUFS_TO_BANKS) {
